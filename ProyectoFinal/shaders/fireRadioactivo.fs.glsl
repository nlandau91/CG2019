...
void main(){
  ...
  // return;
  vec4 n =  texture(material.texture0, v_uv_anim.xy + d2.rg);
  n += grad;
  fragColor = vec4(n.r,n.r,n.r,n.r);
  if(miliseg==0.0){
    fragColor *= mix( vec4(material.color1,1.0), vec4(material.color2,1.0), v_uv.y );
  }
  else{
    osc=(sin(miliseg/1000.0)+1.0)/2.0;
    if(osc<0.1)osc=0.1;
    fragColor *= mix( osc*vec4(material.color1,1.0), osc*vec4(material.color2,1.0), osc*v_uv.y );
  }
